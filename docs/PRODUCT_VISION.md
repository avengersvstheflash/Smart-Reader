...
5. Each source unit → **one Smart Chapter**, output **240–500 words per Smart Chapter (dynamic)**
   (5:1 to 8:1 compression ratio, chosen per source by the system). The output length is assessed
   dynamically per unit — the UI handles varied outputs gracefully.
...
The compression ratio is not a fixed target. It is an observed outcome. Dense technical content preserves every claim at 5:1. Accessible narrative compresses to 8:1 or beyond. The system assesses each source unit and chooses its target; the ratio is the result, not the input.
...
- Smart Chapter target: 240–500 words per Smart Chapter (dynamic)
- Compression ratio: **5:1 to 8:1 compression ratio, chosen per source by the system**
- Hard bounds: clamped dynamically between 150 words floor and source_words * 0.25 ceiling
- If a source unit is only 800 words, output ~150–200 words — short
...
- **their own reading length** (target 240–500 words dynamic)
